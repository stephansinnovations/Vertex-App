...
// screen for when you want to watch it work and manage the connection.

export default function JarvisBuild({ isOpen, onClose }) {
  const cfg = getAgentConfig();
  const [url, setUrl] = useState(cfg.url);
  const [secret, setSecret] = useState(cfg.secret);
  const [needsSetup, setNeedsSetup] = useState(!isAgentConfigured());
  ...
  const saveConfig = () => {
    ...
    setAgentConfig(url.trim(), secret.trim());
    setNeedsSetup(false);
  };
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col"
      ...
      <div className="flex items-center gap-3 px-5 pt-14 pb-3 border-b border-white/10 flex-shrink-0">
        ...
        {!needsSetup && <button onClick={() => setNeedsSetup(true)} className="text-white/50 hover:text-white text-xs px-2 py-1">Setup</button>}
      ...
      {needsSetup ? (
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-4 max-w-md mx-auto w-full">
          ...
          <button onClick={saveConfig} disabled={!url.trim() || !secret.trim()}
            className="w-full bg-sky-500 text-white font-semibold py-3 rounded-xl disabled:opacity-40">Connect</button>
        </div>
      ) : (
        <>
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-2.5">
            ...
            {running && <div className="flex items-center gap-2 text-sky-300/70 text-xs px-2"><Loader2 className="w-3.5 h-3.5 animate-spin" /> Jarvis is on it…</div>}
            <div ref={bottomRef} />
          </div>
          <div className="px-4 py-3 border-t border-white/10 flex-shrink-0 flex items-end gap-2">
            <textarea value={input} onChange={e => setInput(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); send(); } }}
              placeholder="e.g. Add a dark-mode toggle to the home screen" rows={1} disabled={running}
              className="flex-1 bg-black/40 border border-white/15 rounded-2xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-sky-400 resize-none" style={{ maxHeight: 120 }} />
            <button onClick={send} disabled={running || !input.trim()} className="p-2.5 rounded-2xl flex-shrink-0 bg-sky-500 text-white disabled:opacity-30"><Send className="w-4 h-4" /></button>
          </div>
        </>
      )}

      {pending && <ApprovalModal pending={pending} password={password} setPassword={setPassword} onRespond={respond} />}
    </div>
  );
}

// Shared approval prompt — exported so the conversational Jarvis reuses the exact UI.
...
function BuildBubble({ m }) {
  if (m.type === 'user') return <div className="flex justify-end"><div className="max-w-[80%] px-4 py-2.5 rounded-2xl rounded-tr-sm text-sm bg-sky-500 text-white whitespace-pre-wrap">{m.text}</div></div>;
  if (m.type === 'say') return <div className="max-w-[88%] px-4 py-2.5 rounded-2xl rounded-tl-sm text-sm bg-white/10 text-white/90 whitespace-pre-wrap">{m.text}</div>;
  if (m.type === 'status') return <div className="text-xs text-sky-300/70 px-2">{m.text}</div>;
  if (m.type === 'tool') return <div className="text-xs text-white/45 font-mono px-2 truncate">🔧 {m.text}</div>;
  if (m.type === 'error') return <div className="max-w-[88%] px-4 py-2.5 rounded-2xl text-sm bg-red-900/60 text-red-200 whitespace-pre-wrap">{m.text}</div>;
  if (m.type === 'done') return (
    <div className="max-w-[90%] rounded-2xl rounded-tl-sm p-4 bg-white/10 border border-white/10 space-y-2">
      ...
      {m.changed && m.branch && m.branch !== 'main' && (
        <a href={`${REPO_WEB}/tree/${m.branch}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-xs text-sky-300 hover:text-sky-200">
          <ExternalLink className="w-3.5 h-3.5" /> Preview branch: {m.branch} (Vercel is building it)
        </a>
      )}
      {m.changed && m.branch === 'main' && <p className="text-xs text-sky-300/80">Pushed to main — deploying live.</p>}
    </div>
  );
  return null;
}
